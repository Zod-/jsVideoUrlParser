(function (global, factory) {
  ...
  var combineParams$d = util.combineParams,
      getTime$7 = util.getTime;

  function Facebook() {
    this.provider = 'facebook';
    this.alternatives = [];
    this.defaultFormat = 'long'; //List of formats and their functions
  ...
  Facebook.prototype.parseUrl = function (url) {
    var match = url.match(/^((?:https?:\/\/)?(?:www.|m.)?facebook.com\/(.*?videos(.*?)\/(\d+)(.*?)|(watch\/\\?(.*?)v=(\d+))))$/i);
  ...
  Facebook.prototype.createLongUrl = function (vi, params) {
    var url = ''; //Create the url depending on the media type
