(function (global, factory) {
  ...
  function Facebook() {
    ...
    this.provider = 'facebook'; //Alternatives such as shortened versions e.g. youtube.com, youtu.be

    this.alternatives = [];
    this.defaultFormat = 'full'; //List of formats and their functions
  ...
  Facebook.prototype.parse = function (url, params) {
    ...
    var matchVideoId = url.match(/videos\/(\d+)/);
    result.id = matchVideoId ? matchVideoId[1] : params.v;
    var matchPageId = url.match(/(\d+)\/videos/);
    result.pageId = matchPageId ? matchPageId[1] : undefined; //Return nothing when parsing failed
