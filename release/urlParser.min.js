...
urlParser.bind({provider:"twitch",parse:function(a){var b,c,d,f,e={};d=(b=a.match(/twitch\.tv\/(\w+)(?:\/(.)\/(\d+))?/i))?b[1]:void 0;f=b?b[2]:void 0;c=b?b[3]:void 0;if(d=(b=a.match(/(?:\?channel|\&utm_content)=(\w+)/i))?b[1]:d)return c?(e.mediaType="video",e.id=c,e.idPrefix=f):e.mediaType="stream",e.channel=d,e},create:function(a){a=op.videoInfo;return"stream"===a.mediaType?"https://twitch.tv/{0}".format(a.channel):"https://twitch.tv/{0}/{1}/{2}".format(a.channel,a.idPrefix,a.id)}});
urlParser.bind({provider:"vimeo",alternatives:["vimeopro"],parse:function(a){return(a=(a=a.match(/(?:\/(?:channels\/[\w]+|(?:album\/\d+\/)?videos?))?\/(\d+)/i))?a[1]:void 0)?{mediaType:"video",id:a}:void 0},create:function(a){return"https://vimeo.com/{0}".format(op.videoInfo.id)}});
...
a.videoInfo;if("playlist"===b.mediaType)return"https://www.youtube.com/playlist?feature=share&list={0}".format(b.playlistId);b.playlistId?(a="https://www.youtube.com/watch?v={0}&list={1}".format(b.id,b.playlistId),b.playlistIndex&&(a+="&index={0}".format(b.playlistIndex))):a="short"===a.format?"https://youtu.be/{0}".format(b.id):"https://www.youtube.com/watch?v={0}".format(b.id);b.startTime&&(a+="#t={0}".format(b.startTime));return a}});
