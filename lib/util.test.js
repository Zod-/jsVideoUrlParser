...
test('getQueryParams: ?foo=bar', () => {
  expect(getQueryParams('http://foo.bar/test?foo=bar')).toEqual({ foo: 'bar' });
});
...
test('getQueryParams: #foo', () => {
  expect(getQueryParams('http://foo.bar/test#foo')).toEqual({ foo: '' });
});
...
test('combineParams: {foo:\'bar\'}', () => {
  expect(combineParams({ foo: 'bar' })).toEqual('?foo=bar');
});

test('combineParams: {foo:\'bar\',faz:\'baz\'}', () => {
  expect(combineParams({ foo: 'bar', faz: 'baz' })).toEqual('?faz=baz&foo=bar');
});

test('combineParams: {foo: \'bar\',faz: \'baz\',fiz: \'biz\'}', () => {
  expect(combineParams({ foo: 'bar', faz: 'baz', fiz: 'biz' })).toEqual('?faz=baz&fiz=biz&foo=bar');
});

test('combineParams: {foo:\'bar\'} w/ hasParams', () => {
  expect(combineParams({ foo: 'bar' }, true)).toEqual('&foo=bar');
});

test('combineParams: {foo:\'bar\',faz:\'baz\'} w/ hasParams', () => {
  expect(combineParams({ foo: 'bar', faz: 'baz' }, true)).toEqual('&faz=baz&foo=bar');
});

test('combineParams: {foo: \'bar\',faz: \'baz\',fiz: \'biz\'} w/ hasParams', () => {
  expect(combineParams({ foo: 'bar', faz: 'baz', fiz: 'biz' }, true)).toEqual('&faz=baz&fiz=biz&foo=bar');
});