...
test('SoundCloud: undefined', () => {
  ...
  expect(newParser().create({ videoInfo: { provider: 'soundcloud' } })).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'soundcloud', mediaType: 'track' } })).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'soundcloud', mediaType: 'playlist' } })).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'soundcloud', mediaType: 'apitrack' } })).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'soundcloud', mediaType: 'apiplaylist' } })).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'soundcloud', mediaType: 'track' }, format: 'embed' })).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'soundcloud', mediaType: 'playlist' }, format: 'embed' })).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'soundcloud', mediaType: 'apitrack' }, format: 'embed' })).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'soundcloud', mediaType: 'apiplaylist' }, format: 'embed' })).toBe(undefined);
});
