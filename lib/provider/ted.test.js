...
test('Ted: undefined', () => {
  ...
  expect(newParser().create({ videoInfo: { provider: 'ted', mediaType: 'video' }, format: 'embed' })).toBe(undefined);
  ...
  expect(newParser().create({ videoInfo: { provider: 'ted', mediaType: 'playlist' }, format: 'embed' })).toBe(undefined);
});
...
test('Ted: video urls', () => {
  testUrls(newParser(), {
    ...
    formats: {
      ...
      embed: 'https://embed.ted.com/talks/shawn_achor_the_happy_secret_to_better_work',
    },
    urls: [
...
test('Ted: playlist urls', () => {
  testUrls(newParser(), {
    ...
    formats: {
      ...
      embed: 'https://embed.ted.com/playlists/171/the_most_popular_talks_of_all',
    },
    urls: [
